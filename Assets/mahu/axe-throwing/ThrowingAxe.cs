...
public class ThrowingAxe : UdonSharpBehaviour
{
    ...
    // and exit events when the axe is picked up due to the number of colliders on this object
    ...
    public void OnTriggerEnter(Collider other)
    {
        if (Networking.IsOwner(gameObject))
        {
            ...
            {
                //_LogInfo("Axe is entering playable area.");
                inPlayableZone = true;
            }
        }
    }
    ...
    public void OnTriggerExit(Collider other)
    {
        if (Networking.IsOwner(gameObject))
        {
            if (other == game.AxeHoldZone)
            {
                //_LogInfo("Axe is exiting playable area.");
    ...
    {
        //_LogInfo($"!! Collision '{collision.contacts[0].thisCollider.material.name}' on '{collision.contacts[0].otherCollider.material.name}')");
            ...
            && collision.contacts[0].otherCollider.material.name.StartsWith("axemetal"))
        {
            ...
            {
                clangTime = Time.time + Mathf.Min(UnityEngine.Random.Range(0.2f * Mathf.Sqrt(clangs) + 0.2f, 0.7f * clangs + 0.7f), 10f);
                clangs = 0;
                //_LogInfo($"play clang {Networking.GetUniqueName(gameObject)} on {Networking.GetUniqueName(collision.rigidbody.gameObject)}");
    ...
    private void Throw()
    {
        ...
        else
        {
            if (Networking.LocalPlayer.IsUserInVR())
            {
                ...
                if (rb.angularVelocity.magnitude > optimalComputedAngularVel * 1.1f)
                {
                    helpHint += "Try less spin,";
                }
                else if (rb.angularVelocity.magnitude < optimalComputedAngularVel * 0.9f)
                {
                    helpHint += "Try more spin,";
                }
            }
            else
            {
        ...
        if (rb.velocity.magnitude > OPTIMAL_VELOCITY * 1.25f)
        {
            helpHint = "Try throwing slower,";
        }
        else if (rb.velocity.magnitude < OPTIMAL_VELOCITY * 0.75f)
        {
            helpHint = "Try throwing faster,";
        }

        debugLog._Info($"If the player misses, the help hint is {helpHint}");
    ...
    public void _StickAtResult()
    {
        ...
        else
        {
            ...
            gameText._PlayHelpText("FOUL! Over line", 0.5f);
        }
    }
    ...
    private bool ThrowSimulationV2(out float finalTime, out Vector3 finalPosition, out Quaternion finalRotation)
    {
        ...
        if ((closestBoardIntersection - posBeforeAdj).magnitude > radius * 2f)
        {
            ...
            helpHint += "Axe must strike your own board,";
            return false;
        }
        ...
        if (!sticks)
        {
            helpHint += "Axe edge must cut into board,";
        }
        ...
        if (sticks && Mathf.Abs(cutError) > 1e-5)
        {
            ...
            else
            {
                ...
                if (alignedHeadAngle < minCutAngle)
                {
                    helpHint += "Under rotation,handle hit board,try backing up,";
                }

                if (alignedHeadAngle > maxCutAngle)
                {
                    helpHint += "Over rotation,try moving closer,";
                }
    ...
    public void _PlayNostickHelp()
    {
        debugLog._Info($"Nostick help - stuck: {stuck}, hitBoard {hitBoard}, helpHint {helpHint}");

        if (stuck)
            return;

        if (Time.time - lastHelpTime < 8f)
            return;

        if (!string.IsNullOrEmpty(helpHint))
        {
            var tipTextSplit = helpHint.Trim(',').Split(',');
            if (tipTextSplit.Length > 0)
            {
                var tipText = tipTextSplit[UnityEngine.Random.Range(0,tipTextSplit.Length)];
                lastHelpTime = Time.time;
                gameText._PlayHelpText(tipText);
            }
        }
    }
}
